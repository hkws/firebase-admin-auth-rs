$(function () {
  ...
  getUidButton.click(function (event) {
    event.preventDefault()
    $('#callouts').append($('<p>').text(`[request token] ${userIdToken}`))
    $.ajax(baseUrl + '/uid', {
      headers: {
      ...
      .then(function (_, _, jqXHR) {
        $('#callouts').append(
          $('<p>').text(
            `[response: info] ${jqXHR.status} ${jqXHR.statusText}: ${jqXHR.responseText}`,
          ),
        )
      })
      .catch(function (_, _, jqXHR) {
        $('#callouts').append(
          $('<p>').text(
            `[reponse: error] ${jqXHR.status} ${jqXHR.statusText}: ${jqXHR.responseText}`,
          ),
        )
      })
  })
